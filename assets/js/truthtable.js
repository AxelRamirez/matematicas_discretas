...
function construct() {
	...
	if(formulas=='') {return alert("Tienes que ingresar una fórmula.");};
	var r = badchar(formulas);
	if(r>=0) {return alert("La cadena que ingresó contiene el siguiente símbolo no reconocido: "+formulas[r]);};
	...
	if(trees.filter(function(a) {return a.length==0;}).length>0) { // checks if any formulas are still malformed
		return alert("Una de las fórmulas que ingresó no está bien formulada.");
	}
